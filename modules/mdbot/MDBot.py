...
def b30(md_uid):
    ...
    for i in range(30):
        if i >= len(songs):
            break
        message += '\n{0}({1}) {2}'.format(songs[i][1]['name'], songs[i][1]['diffdiff'], songs[i][1]['acc'])
    return message
...
def uid2name(uid):
    ...
    tree = etree.HTML(response.text)
    name = tree.xpath('//*[@id="app"]/section/div/div/div/section/div/div/div[1]/div/div[1]/h1')[0].text
...
def update_musics():
    ...
    for level in level_dif:
        level_dif[level] = sorted(level_dif[level].items(), key=lambda x: x[1]['dif'], reverse=True)
    ...
    for uid in musics_data:
        if musics_data[uid]['name'] == close_name[0]:
            ...
            if musics_data[uid]['difficulty'][0] != '0':
                message += '\n萌新:{0}({1:.2f})'.format(musics_data[uid]['difficulty'][0],
                                                        musics_data[uid]['diffdiff'][0])
            if musics_data[uid]['difficulty'][1] != '0':
                message += '\n高手:{0}({1:.2f})'.format(musics_data[uid]['difficulty'][1],
                                                        musics_data[uid]['diffdiff'][1])
            if musics_data[uid]['difficulty'][2] != '0':
                message += '\n大触:{0}({1:.2f})'.format(musics_data[uid]['difficulty'][2],
                                                        musics_data[uid]['diffdiff'][2])
            if musics_data[uid]['difficulty'][3] != '0':
                message += '\n里谱:{0}({1:.2f})'.format(musics_data[uid]['difficulty'][3],
                                                        musics_data[uid]['diffdiff'][3])
            message += '\n谱师:'
